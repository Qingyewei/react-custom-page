...
import { SystemEvent } from './system_event_listener';
...
function Request(axiosConfig: AxiosRequestConfig<any>, customOptions?: any, loadingOptions?: any) {
  const service = axios.create({
    baseURL: import.meta.env.VITE_API_URL_PREFIX, // 设置统一的请求前缀
    timeout: 10000, // 设置统一的超时时长
    // withCredentials: true, // 跨域cookies写不进去，设置为true就可以
  });
  ...
  service.interceptors.request.use(
    (config) => {
      ...
      if(Csrf.token){
        config.headers!["x-csrf-token"] = Csrf.token as string;
      }
...
export function httpErrorStatusHandle(error: {
  ...
  if (error && error.response) {
    switch (error.response.status) {
      ...
      case 401:
        ...
        isMessage = false
        SystemEvent.emit(CONSTANTS.EVENT.CAS_AUTH_401, error.response);
        break;
      case 403:
        message = '您没有权限操作！';
        break;
  ...
  isMessage && Message.error(message);
}
